// Ћабораторна€ 7, задача 1(вариант 10). ¬ыполнена: 

/* из дев€тичной в семнадцатеричную (без массива) */
...
int main()
{
	...
	cout << "ѕрограмма осуществл€ет перевод чисел из дев€тичной в семнадцатеричную систему счислени€ (без массива).";
	...
	cout << "\n¬аше число в семнадцатиричной сс: ";
...
char* from_ten_to_seventeenfold(double value_10, char* arr) {
	...
	for (j = 0; del > 0; j++) {
		...
		whole = del * 17; // цела€ часть от делени€
...
double get_value_ninefold() {
	while (true) {
		double element;
		cout << "\n¬ведите число в дев€тичной системе счислени€: ";
		cin >> element;
		if (element < 0)
			cout << "-";
